...
SPDetector::SPDetector(string model_path){
    SP_interpreter = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(model_path.c_str()));
    MNN::ScheduleConfig config;
    config.type = MNN_FORWARD_CPU;
    config.backupType = MNN_FORWARD_CPU;
    MNN::BackendConfig backendConfig;
    backendConfig.precision = (MNN::BackendConfig::PrecisionMode) 0;
    backendConfig.power = (MNN::BackendConfig::PowerMode) 1;
    config.backendConfig = &backendConfig;

    SP_session = SP_interpreter->createSession(config);
    input_tensor = SP_interpreter->getSessionInput(SP_session, nullptr);

}

SPDetector::~SPDetector(){
    SP_interpreter->releaseModel();
    SP_interpreter->releaseSession(SP_session);
...
void SPDetector::detect(cv::Mat& img, vector<cv::KeyPoint>& res_keypoints, cv::Mat& res_descriptors){
    ...
    SP_interpreter->resizeTensor(input_tensor, {1, 1, input_dims_.dim3, input_dims_.dim4});
    SP_interpreter->resizeSession(SP_session);
    std::shared_ptr<MNN::CV::ImageProcess> pretreat(
        MNN::CV::ImageProcess::create(MNN::CV::GRAY, MNN::CV::GRAY, nullptr, 0, &norm_vals_, 1)
    );
    pretreat->convert(img.data, img.cols, img.rows, img.step[0], input_tensor);    
    auto end = std::chrono::steady_clock::now();
    std::chrono::duration<double> d = end - start;
    cout << "preprocess need " << d.count() * 1000 << " ms." << endl;

    start = std::chrono::steady_clock::now();
    SP_interpreter->runSession(SP_session); // CPU推理同步阻塞，CUDA推理时异步不等待，在getSessionOutput时等待推理完成

    MNN::Tensor* preds0_tensor = SP_interpreter->getSessionOutput(SP_session, output_name_[0].c_str());
    MNN::Tensor preds0_tensor_host(preds0_tensor, preds0_tensor->getDimensionType());
    preds0_tensor->copyToHostTensor(&preds0_tensor_host);

    MNN::Tensor* preds1_tensor = SP_interpreter->getSessionOutput(SP_session, output_name_[1].c_str());
    MNN::Tensor preds1_tensor_host(preds1_tensor, preds1_tensor->getDimensionType());
    preds1_tensor->copyToHostTensor(&preds1_tensor_host);

    float* output_score = preds0_tensor_host.host<float>();
    float* output_desc = preds1_tensor_host.host<float>();

    end = std::chrono::steady_clock::now();
    d = end - start;
    cout << "run need " << d.count() * 1000 << " ms." << endl;

    start = std::chrono::steady_clock::now();
    bool pFlag = process_output(output_score, output_desc, res_keypoints, res_descriptors);
    end = std::chrono::steady_clock::now();
    d = end - start;
    cout << "postprocess need " << d.count() * 1000 << " ms." << endl;
}
...
int demo01(){
    ...
    string img0_path = "../image/3.jpg";
    string img1_path = "../image/4.jpg";
    ...
    SPDetector superpoint("../weights/superpoint_v1.mnn");
